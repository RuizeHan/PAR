...
def train_jrdb(data_loader, model, device, optimizer, epoch, cfg):
    ...
    for batch_data in tqdm(data_loader):
        ...
        # forward
        actions_scores, activities_scores, relation_graphs, social_acty_scores, seq_ids, frame_ids, boxes_coords = model(
    ...
    with torch.no_grad():
        for batch_data in tqdm(data_loader):
            ...
            for ii in range(actions_labels.shape[0]):
                action_result[0].append(actions_labels[ii])
                action_result[1].append(actions_in[ii])
                action_ids[0].append(seq_ids[ii])
                action_ids[1].append(frame_ids[ii])
                action_ids[2].append(boxes_coords[ii])
        ...
        all_result = {
            ...
            'seq_frame_ids&boxes_coords': action_ids
        }
        np.save(cfg.result_path + '/all_result_epoch{}.npy'.format(epoch), all_result)
